...
- **주간 퀴즈(2026-09 말 추가, 세 번째 탭)**: "오늘의 시험" 탭 옆에 새로 추가된 탭. **최근 7일(오늘 포함) 안에 등록된 단어 전체**(이미 외운 단어 포함)가 출제 범위인 복습용 4지선다 퀴즈예요. "오늘의 시험"과 같은 화면(퀴즈 카드)·같은 오답 재출제 로직을 공용 엔진(`QUIZ_MODES`)으로 같이 쓰지만, **`correctStreak`·`mastered` 등 학습 기록에는 전혀 반영되지 않는 완전히 독립된 복습**이에요(정답/오답 집계는 그 퀴즈 세션 안에서만 보여주고 저장되지 않음). 매주 등록한 단어를 가볍게 다시 훑어보는 용도로, "오늘의 시험"과 대상 단어·데이터 반영 여부가 다르다는 점만 유의하면 돼요.
