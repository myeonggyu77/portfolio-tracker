...
- **사전 자동조회(품사·뜻·예문, 선택 기능, 2026-09 확장, 2026-09 말 대대적 개편)**: 단어 등록 폼에서 **단어 입력 후 다른 칸으로 포커스를 옮기면(blur) 자동으로** 품사·뜻·예문을 채워요. 언어 아이콘 버튼(`wf-dict-btn`)으로 언제든 수동 재조회도 가능해요.
  ...
  - **품사 대체(fallback) 소스: Datamuse API(2026-09 말 추가, api.datamuse.com, 가입·키 불필요)**: dictionaryapi.dev가 예고 없이 다운되는 일이 실제로 있었어요(직접 겪은 사례: Cloudflare 522 장애로 몇 시간 동안 응답 자체가 안 됨 — 우리 코드 문제 아니라 그 사이트 자체 장애였음, `dict-diag`로 직접 확인). dictionaryapi.dev에서 품사를 못 가져오면 `fetchPosViaDatamuse()`가 한 번 더 품사만 조회해요. Datamuse는 예문은 제공하지 않아서, dictionaryapi.dev가 죽어있는 동안엔 예문만 비어있을 수 있어요(뜻·품사는 계속 채워짐). dictionaryapi.dev가 복구되면 예문도 자동으로 다시 채워져요.
